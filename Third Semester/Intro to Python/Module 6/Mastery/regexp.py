...
#Author: Kc Poland
#Description: This is the regexp.py test.
...
x = re.search(r"\bhello\b|\bhero\b", text)
...
x = re.search(r"\bgrand\b|\bgrind\b", text)
...
x = re.search(r"\bGood Day\b|\bGood Night\b", text)
...
x = re.search(r"\b[1-2][0-9][0-9]\b", text)
...
x = re.search(r"\bq[^u]", text)  #Includes only words that "start with" q. Instructions may contain a typo?
...
x = re.search(r"\b\d\d\d-\d\d-\d\d\d\d\b", text)
...
x = re.search(r"\w+@\w+\.com\b|\w+@\w+\.net\b|\w+@\w+\.edu\b", text)
...
s = re.findall("[a-zA-Z]{4}\w*", textVar)
